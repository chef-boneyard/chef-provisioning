...
class Chef
class Provider
class MachineDirectory < Chef::Provider::LWRPBase
  ...
  action :upload do
    machine.upload_directory(action_handler, new_resource.local_path, new_resource.path)
  ...
  action :download do
    machine.download_directory(action_handler, new_resource.path, new_resource.local_path)
  end

  action :delete do
    machine.delete_directory(action_handler, new_resource.path)
  end
