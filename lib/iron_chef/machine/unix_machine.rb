  ...
  class Machine
    class UnixMachine < BasicMachine
      ...
      def delete_file(provider, path)
        if file_exists?(path)
          provider.converge_by "rm -f #{path} on #{node['name']}" do
      ...
      def file_exists?(path)
        transport.execute("ls #{path}").stdout != ''
      end
      ...
      def set_attributes(provider, path, attributes)
        if attributes[:mode] || attributes[:owner] || attributes[:group]
          ...
          if attributes[:mode] && current_attributes[:mode] != attributes[:mode]
            provider.converge_by "changing mode of #{path} on #{node['name']} from #{current_attributes[:mode].to_i(8)} to #{attributes[:mode].to_i(8)}" do
              transport.execute("chmod #{attributes[:mode].to_i(8)} #{path}").error!
            end
          end
          if attributes[:owner] && current_attributes[:owner] != attributes[:owner]
            provider.converge_by "changing group of #{path} on #{node['name']} from #{current_attributes[:owner]} to #{attributes[:owner]}" do
              transport.execute("chown #{attributes[:owner]} #{path}").error!
            end
          end
          if attributes[:group] && current_attributes[:group] != attributes[:group]
            provider.converge_by "changing group of #{path} on #{node['name']} from #{current_attributes[:group]} to #{attributes[:group]}" do
